...
(function () {
  ...
  function getFieldText(label) {
    ...
    for (const block of blocks) {
      ...
      if (strongText.toLowerCase() !== label.toLowerCase()) continue;
    ...
    const match = bodyText.match(
      new RegExp(`${escaped}\\s+(.+?)(?=\\s+(?:Story by|Release|ISBN-13|Trim Size|Imprint|Length|Series|Category|Age Rating)\\b|$)`, 'i')
    );
    return match ? normalizeText(match[1]) : '';
  }

  function getPageCount() {
    const raw = getFieldText('Length');
  ...
  function getAge() {
    ...
    if (/^Teen Plus$/i.test(raw)) return 'Age Rating_Teen Plus';
