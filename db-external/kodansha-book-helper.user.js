...
function getField(label) {
  for (const item of document.querySelectorAll('.volume-info__item')) {
    ...
    if (
      labelEl &&
      valueEl &&
      normalizeText(labelEl.textContent).toLowerCase() === label.toLowerCase()
...
function getSynopsis() {
  return richPayloadFromElement(document.querySelector('.volume__hero__description'));
}

function getAuthors() {
  const root = document.querySelector('.volume__hero__creator');
  if (!root) return '';

  const links = [...root.querySelectorAll('a')].map(el => normalizeText(el.textContent)).filter(Boolean);
  return links.length ? joinNames(links) : joinNames([root.textContent]);
}

function getPage() {
  return getField('Pages').match(/\d+/)?.[0] || '';
}
...
function getISBN() {
  ...
  document.querySelectorAll('.volume-info__cell').forEach(cell => {
    const text = normalizeText(cell.textContent).toLowerCase();
    const isbnItem = [...cell.querySelectorAll('.volume-info__item')].find(item =>
      normalizeText(item.querySelector('.volume-info__label')?.textContent).toLowerCase() === 'isbn'
    );
    const isbn = normalizeText(isbnItem?.querySelector('.volume-info__value')?.textContent)
      .replace(/[^0-9X]/gi, '')
      .toUpperCase();

    if (!isbn) return;
    if (text.includes('print release')) printISBN = isbn;
    if (text.includes('digital release')) digitalISBN = isbn;
  });
...
function getReleaseDate() {
  return toISODate(getField('Print Release') || getField('Digital Release'));
}
...
function getAge() {
  ...
  if (raw.includes('13') || raw.includes('12')) return 'Age Rating_Teen';
...
function getPublisher() {
  return 'Publisher_Kodansha Comics USA';
}

function getImprint() {
  return 'Imprint_Kodansha Comics USA';
}
