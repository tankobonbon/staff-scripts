...
(function () {
    ...
    function getCategories() {
        const container = document.querySelector('[data-cy="info-box-categories"]');
        if (!container) return [];

        const cats = Array.from(
            container.querySelectorAll('a[href*="/series?category="]')
        )
        .map(a => normalizeWhitespace(a.textContent || ''))
        .filter(Boolean);
    ...
    function parseCategoryScore(titleText) {
        const m = (titleText || '').match(/Score:\s*([0-9.]+)/i);
    ...
    function getCategoryObjects() {
        ...
        for (const a of links) {
            const name = normalizeWhitespace(a.textContent || '');
            ...
            out.push({
                name,
                score: parseCategoryScore(a.getAttribute('title') || '')
            });
        }
    ...
    async function getCategoriesExpanded() {
        const container = document.querySelector('[data-cy="info-box-categories"]');
        if (!container) return [];

        let cats = getCategories();
        const beforeCount = cats.length;

        const showLink = Array.from(container.querySelectorAll('a, button'))
        .find(el => /show all|show/i.test(normalizeWhitespace(el.textContent || '')));
    ...
    async function getCategoriesExpandedObjects() {
        const container = document.querySelector('[data-cy="info-box-categories"]');
        if (!container) return [];

        let objects = getCategoryObjects();
        ...
        while (Date.now() - start < timeoutMs) {
            ...
            objects = getCategoryObjects();

            if (objects.length > beforeCount) {
                // if count increased, wait until it stabilizes a bit
                if (objects.length === lastCount) {
                    stableHits++;
                } else {
                    stableHits = 0;
                }
    ...
    function getTopCategoryNames(categoryObjects, limit = MAX_THEMES) {
    ...
    function makeButton(label, action, isWide = false) {
        ...
        const isCategories = action === 'copy-categories';
        const isTopThemes = action === 'copy-top-categories';
        ...
        applyStyles(btn, {
            ...
            ? '1px solid #0f766e'
            : '1px solid #666',
            background: isCategories
            ? 'linear-gradient(#7c3aed, #5b21b6)'
            : isTopThemes
            ? 'linear-gradient(#14b8a6, #0f766e)'
            : 'linear-gradient(#4a4a4a, #2f2f2f)',
            color: '#fff',
            borderRadius: '3px',
            padding: '6px 8px',
            cursor: 'pointer',
        ...
        btn.addEventListener('mouseenter', () => {
            btn.style.background = isCategories
                ? 'linear-gradient(#8b5cf6, #6d28d9)'
            : isTopThemes
                ? 'linear-gradient(#2dd4bf, #0f766e)'
            : 'linear-gradient(#575757, #353535)';
        });
    ...
    function createBox() {
        ...
        const buttons = [
            ...
            ['Copy Categories/Themes', 'copy-categories'],
            ['Copy Top Themes (128)', 'copy-top-categories'],
        ];
        ...
        box.addEventListener('click', async (e) => {
            ...
            if (action === 'copy-categories') {
                const before = getCategories().length;
                let cats = await getCategoriesExpanded();

                if (!cats.length) {
                    status.textContent = 'No categories found 😭';
                    return;
                }
                ...
                status.textContent = ok
                    ? `Copied ${cats.length} categories${cats.length > before ? ' after expanding' : ''}.`
    : 'Copy failed 😭';

    return;
}

        if (action === 'copy-top-categories') {
            const categoryObjects = await getCategoriesExpandedObjects();
            const top = getTopCategoryNames(categoryObjects, MAX_THEMES);
            ...
            const ok = await copyText(top.join('\n'));

            status.textContent = ok
                ? `Copied top ${top.length} themes for Shopify.`
    : 'Copy failed 😭';
